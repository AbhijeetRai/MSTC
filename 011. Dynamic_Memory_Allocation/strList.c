...
STRLIST get_list();
int add_string(char* string_to_add, char*** list);
...
STRLIST get_list() {
    ...
    STRLIST list = (char**)malloc(sizeof(char*));
    return list;
}
...
int add_string(char* string_to_add,STRLIST* list) {
    ...
    length_of_list += 1;
    STRLIST pCharList=*list;
    pCharList = (char**)realloc(pCharList, sizeof(char*) * length_of_list);
    *(pCharList + (length_of_list - 1)) = string_to_add;
    *list=pCharList;
